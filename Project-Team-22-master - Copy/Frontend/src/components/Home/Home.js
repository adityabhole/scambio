...
class Home extends Component {
    ...
    loadMyJunks = async () => {
        ...
        http.getMyJunks(data1).then(data => {
            if(data){
    ...
    junkList = () => {
        const list = this.state.pjunks.map((junk) =>
            <div key={junk._id}>
                <Junk _id={junk._id} title={junk.Name} description={junk.ItemDesciption} imgUrl={junk.ImageURL} condition={junk.ItemCondition}/>
            </div>
        );
        return (list);
    }
    ...
    renderSelectedItem = () => {
        ...
        if (sid == null){
            ...
            const i = this.state.myjunks.filter(j => sid.value == j._id).map((j) =>
                <div className="card preview-card">
                    <img className="card-img-top" src={ j.ImageURL } alt="Junk"></img>
